...
fn v1_dynamic_modes_have_explicit_event_and_analytical_decay() {
    ...
    let after_irregular_step = model
        .process_transition(&stepped, &parameters, &input(0.1), 0.4)
        .unwrap();
    let decayed = model
        .process_transition(&after_irregular_step, &parameters, &input(0.1), 1.6)
        .unwrap();
    ...
    let mut negative_event = input(0.1);
    negative_event.values.insert(
        "delta_log10_activity".into(),
        InputValue {
            value: -1.0,
            unit: "activity".into(),
    ...
    let negative = model
        .process_transition(&initial, &parameters, &negative_event, 0.0)
        .unwrap();
    assert!((negative.values[fast] + 0.02).abs() < 1e-14);
}
