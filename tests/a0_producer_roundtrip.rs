...
fn legacy_read<T: rust_electroanalysis_cli::VersionedArtifact>(path: &Path, value: &T) -> T {
    let mut json = serde_json::to_value(value).expect("serialize legacy candidate");
    json["schema_version"] = serde_json::Value::from(1);
    json.as_object_mut()
        .expect("object")
        .remove("artifact_kind");
    fs::write(path, json.to_string()).expect("write legacy fixture");
    read_artifact(path).expect("read legacy fixture")
}
...
fn mhi_t02d_legacy() {
    ...
    let path = temp_dir().join("health_baseline_v1.json");
    fs::write(&path, value.to_string()).unwrap();
    let baseline: rust_electroanalysis_cli::results::SensorHealthBaseline =
        read_artifact(&path).expect("schema-1 legacy fixture");
...
fn mhi_t02f_producer_roundtrip() {
    ...
    legacy_read(&root.join("transient-v1.json"), &transient);
    ...
    let observations = roundtrip(&root.join("observations.json"), &observations);
    legacy_read(&root.join("observations-v1.json"), &observations);
    ...
    legacy_read(&root.join("calibration-v1.json"), &calibration);
    ...
    let model = roundtrip(&root.join("model.json"), &model);
    legacy_read(&root.join("model-v1.json"), &model);
    ...
    let signal: SignalAnalysisReport = roundtrip(&root.join("signal.json"), &signal);
    legacy_read(&root.join("signal-v1.json"), &signal);
    ...
    legacy_read(&root.join("mechanism-compare-v1.json"), &mechanism);
    ...
    legacy_read(&root.join("assessment-v1.json"), &assessment);
    let trend: HealthTrendReport =
        health_trend_report("a0-trend", Vec::new(), assessment.provenance.clone());
    let trend = roundtrip(&root.join("health-trend.json"), &trend);
    legacy_read(&root.join("health-trend-v1.json"), &trend);
