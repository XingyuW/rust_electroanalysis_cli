...
pub fn build_hypothesis_assessment_hash_view(
    ...
    view.amplitude_assessments.sort_by(|left, right| {
        (&left.predicted_requirement_id, &left.observed_requirement_id).cmp(&(
    ...
    view.repeatability_assessments.sort_by(|left, right| {
        left.requirement_ids.cmp(&right.requirement_ids)
    });
