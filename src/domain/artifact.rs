...
/// Identifies the scientific payload carried by a cross-workflow JSON artifact.
...
/// Contract implemented by JSON values passed from one workflow to another.
///
/// The root payload retains its established `schema_version` field. New files
/// additionally receive an `artifact_kind` header; legacy files without this
/// header can be migrated only when their typed expected schema is supported.
...
pub enum ArtifactError {
    ...
    #[error("artifact {path} has no schema_version")]
    MissingSchemaVersion { path: PathBuf },
    #[error("artifact {path} has an invalid schema_version")]
...
pub fn read_artifact<T: VersionedArtifact>(path: &Path) -> Result<T, ArtifactError> {
    ...
    if text.contains("NaN") || text.contains("Infinity") || text.contains("-Infinity") {
        return Err(ArtifactError::NonFiniteValue { path: path.into() });
    }
...
pub fn write_artifact<T: VersionedArtifact>(
    ...
    object.insert(
        ...
        Value::String(T::ARTIFACT_KIND.as_str().to_string()),
    );
    validate_value::<T>(path, &value)?;
...
fn validate_value<T: VersionedArtifact>(path: &Path, value: &Value) -> Result<(), ArtifactError> {
    ...
    } else if schema == T::CURRENT_SCHEMA_VERSION && !T::LEGACY_SCHEMA_VERSIONS.contains(&schema) {
        return Err(ArtifactError::IncompatibleKind {
            path: path.into(),
            expected: T::ARTIFACT_KIND,
            actual: None,
...
fn schema_version(path: &Path, object: &Map<String, Value>) -> Result<u32, ArtifactError> {
    let value = object
        .get("schema_version")
        .ok_or_else(|| ArtifactError::MissingSchemaVersion { path: path.into() })?;
    value
        .as_u64()
