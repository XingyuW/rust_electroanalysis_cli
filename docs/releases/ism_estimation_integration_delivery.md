...
The production correction wires the resolved typed binding plan into the shared
compiled model-input execution path. The cumulative files cover estimation
configuration, model compilation/adaptation, environment/event alignment,
EKF/UKF execution, simulation, validation, covariance, reports, migration
fixtures, and permanent integration tests. Public schema/API additions are
optional compiled backend/profile and binding provenance fields, stable-ID
compiled truth, and typed binding errors; Legacy defaults and old serialized
fields remain backward compatible through explicit Serde defaults and honest
Legacy/unavailable identity.

No required command was omitted. The only auxiliary warning was EOF blank-line
output from `git diff --check` on four TOML fixtures.

## Freeze metadata

Approval tag created on the documentation freeze commit:
