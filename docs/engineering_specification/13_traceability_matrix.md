...
| ISM-009 | `model/compiler.rs`, `model/error.rs`, `model/state.rs`, `model/parameter.rs` | declared uncertainty class and covariance-contract enforcement | `tests/model_core.rs`: `covariance_matrix_validation_has_typed_failures`; `tests/model_v1_units_charge_serialization_contract.rs`: `parameter_covariance_nonfinite_entries_are_rejected_with_exact_paths` | The former protects state-covariance nonfinite/asymmetric/non-PSD rejection only. The latter independently protects public parameter-covariance serialization and direct `parameter` covariance coordinates. |
| ISM-010 / ISM-011 / ISM-012 | `model/parameter.rs`, `model/validity.rs`, `model/evidence.rs`, `results/model.rs` | V1 scientific guardrails | `model_components_v1_contract` | Explicit domains only; no universal calibration claim |
| ISM-013 | `model/compiler.rs`, `model/validity.rs`, `model/error.rs` | legacy/typed declaration merge and typed conflict context | `tests/model_v1_applicability_contract.rs`: `legacy_only_constraint_is_migrated_losslessly`; `typed_only_constraint_is_resolved_and_preserved`; `mixed_legacy_and_typed_constraints_survive_with_stable_ordering`; `exact_duplicate_retains_both_provenance_sources_and_conflicts_name_fields`; `conflicting_intervals_and_policies_return_typed_conflict_context` | Schema-4 migration is deterministic, preserves both declaration sources, and rejects competing intervals or policies. |
| ISM-014 | `model/compiler.rs`, `model/validity.rs`, `model/error.rs` | per-constraint Warn/Reject enforcement and unavailable evidence | `tests/model_v1_applicability_contract.rs`: `independent_warn_and_reject_policies_keep_each_constraint_outcome`; `unavailable_warn_and_reject_are_independent_and_ordered` | Warnings and fatal errors retain constraint ID, domain status, subject, and policy. |
| ISM-012 | `model/equilibrium_recognition.rs`, `model/evidence.rs` | status classification and missing/not-applicable evidence | `tests/model_v1_equilibrium_identifiability_contract.rs`: `equilibrium_recognition_classifies_every_v1_status_with_auditable_criteria`; `equilibrium_recognition_preserves_missing_not_applicable_and_unobservable_evidence` | All V1 equilibrium statuses retain criteria, warnings/confidence, and missing evidence. |
| ISM-015 | `model/compiler.rs`, `model/identifiability.rs`, `results/model.rs`, `domain/artifact.rs` | identifiability topology, unit/charge safeguards, and finite public artifacts | `tests/model_core.rs`: `model_artifact_rejects_nonfinite_definition_values`. `tests/model_v1_equilibrium_identifiability_contract.rs`: `identifiability_metadata_distinguishes_one_and_two_active_dynamic_modes`; `optional_requirements_promote_to_active_and_serialized_order_is_stable`. `tests/model_v1_units_charge_serialization_contract.rs`: `nernst_component_rejects_every_invalid_discrete_charge_without_coercion`; `nicolsky_component_validates_target_and_interferent_charges_on_its_actual_path`; `covariate_units_accept_exact_contracts_and_report_precise_mismatches`; `public_serialization_paths_reject_nested_nonfinite_values_without_creating_files`; `analysis_serialization_rejects_each_uncertainty_and_nested_numeric_path`; `applicability_interval_infinity_is_rejected_by_all_public_serializers`; `parameter_covariance_nonfinite_entries_are_rejected_with_exact_paths`; `raw_equilibrium_evidence_nonfinite_values_are_rejected_with_exact_paths` | `model_artifact_rejects_nonfinite_definition_values` protects `model_definition.parameters[0].default_value`; the V1 tests protect exact interval, covariance, and raw-evidence paths plus no-file write failure, without allowing JSON `null` substitution. |

### Compiled-estimation integration traceability

Every integration requirement is recorded against an exact function rather
than only the containing test file.

| Requirement | Test file | Exact test function | Profile / filter / state model / scenario | Protected contract |
|-------------|------------|---------------------|--------------------------------------------|--------------------|
| Runtime custom binding execution and custom target ID | `tests/phase6_estimation.rs` | `custom_flow_drive_binding_executes_in_normal_estimation_runtime` | Compiled Custom / EKF / Activity / environmental flow covariate | `environment:flow` populates `flow_drive`; definition hash, prediction, report provenance, and component observation all succeed |
| Typed source plan and failures | `tests/phase6_estimation.rs` | `compiled_bindings_cover_standard_custom_target_and_constant_sources`; `compiled_bindings_support_named_environment_and_event_field_sources`; `compiled_bindings_reject_typed_target_source_and_unit_failures`; `compiled_bindings_report_missing_and_optional_sources_without_position_assumptions` | Compiled Custom / shared EKF-UKF input path / custom definition | Standard precedence, named series, constants, unit conversion, unsupported/unknown/duplicate/missing/optional handling |
| Legacy parity states, filters, scenarios, and condition/sensitivity | `tests/phase6_estimation.rs` | `compiled_legacy_equivalent_permanent_parity_matrix_covers_states_filters_and_scenarios`; `compiled_legacy_parity_covers_condition_sensitivity_state_for_ekf_and_ukf` | Legacy vs Compiled LegacyEquivalentV1 / EKF and UKF / Activity, ActivityBaseline, ActivityBaselinePolarization, sensitivity/condition | Named state and covariance parity, innovations/NIS, statuses, gating, calibration-domain behavior, and documented `1e-10`/`1e-8` tolerances |
| Irregular and multiple activity events | `tests/phase6_estimation.rs` | `compiled_activity_events_are_applied_once_at_irregular_transitions_with_provenance` | Compiled transition binding / reduced profile / irregular event sequence | Exactly-once event application, deterministic interval aggregation, initial/no-event behavior, dynamic event timestamps |
| All transduction-drive modes | `tests/phase6_estimation.rs` | `compiled_transduction_drive_modes_cover_none_activity_step_event_field_and_failures` | ReducedIsmV1 / transition binding / None, ActivityStep, ExplicitEventField | No impulse, activity-step impulse, event field conversion, missing field, invalid unit, multiple event aggregation |
| Stable truth and metrics | `tests/phase6_estimation.rs` | `compiled_reduced_active_transduction_truth_and_validation_use_stable_state_ids` | ReducedIsmV1 / EKF plus simulation / active transduction | Stable fast/slow/reference/candidate IDs, contributions, ordering, RMSE/MAE/bias/coverage by ID |
| Reports and old configuration/artifact migration | `tests/phase6_estimation.rs` | `estimation_report_matrix_renders_honest_backend_profile_and_custom_definition`; `old_estimation_configuration_fixture_matrix_preserves_legacy_defaults`; `old_estimation_artifact_migration_matrix_keeps_identity_honest_and_deterministic` | Legacy, Compiled LegacyEquivalentV1, Compiled ReducedIsmV1, Custom / EKF; migration fixtures | Human report narrative, custom component/binding/status data, schema defaults, honest old backend identity, deterministic reserialization |

### Final compiled-estimation evidence requirements

| Requirement | Acceptance criterion | Implementation | Test | Result |
|---|---|---|---|---|
| R1 | AC1.1–AC1.7: real target/interferent Nicolsky scenario, active selectivity, signed charges, EKF/UKF normal-path parity, no missing-interferent shortcut, documented tolerance | `tests/phase6_estimation.rs` Nicolsky calibration helper and normal `estimate_experiment` path | `compiled_legacy_equivalent_nicolsky_interferent_parity_runs_ekf_and_ukf` | Pass; Legacy and Compiled + LegacyEquivalentV1 agree within `1e-10` |
| R2 | AC2.1–AC2.6: tracked historical report, simulation, validation, and comparison fixtures deserialize through compatibility paths with honest defaults and deterministic reserialization | `tests/fixtures/estimation_migration/`; `tests/estimation_artifact_migration.rs` | `legacy_state_estimation_report_fixture_migrates`; `legacy_simulation_truth_fixture_migrates`; `legacy_validation_fixture_migrates`; `legacy_filter_comparison_fixture_migrates` | Pass; old artifacts remain Legacy/unavailable rather than Compiled |
| R3 | AC3.1–AC3.5: stable-ID truth omission leaves metrics unavailable without fabricated zero values | `src/estimation/validation.rs` stable-ID lookup; regression fixture removes `reference_offset_v` truth | `compiled_validation_keeps_absent_truth_metrics_unavailable` | Pass; available states score and omitted state has `None` metrics/sample count 0 |
| R4 | AC4.1–AC4.5: duplicate binding rejected with typed project error and preserved context | `src/estimation/ism_adapter.rs` `DuplicateModelInputBinding` context; compiler test fixture | `duplicate_binding_key_returns_typed_error_with_target_and_declaration` | Pass; target ID, declaration, and model ID are pattern-matched |
