...
| ISM-013 / ISM-014 / ISM-015 | `model/compiler.rs`, `model/validity.rs`, `model/error.rs`, `results/model.rs`, `domain/artifact.rs` | merged applicability declarations, per-constraint enforcement, shared finite serialization | `model_components_v1_contract`, `model_core`, `artifact_contract` | Schema-4 compatible additive fields; no workflow integration |
