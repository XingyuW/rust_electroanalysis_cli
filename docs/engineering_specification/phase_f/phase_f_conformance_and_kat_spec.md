...
|---|---|---|---|
| <a id="F-CNF-001"></a>`F-CNF-001` | `F-ARCH-017,F-WIRE-001..009` | Production-schema KATs use complete schema-valid bytes and verify parsing, JCS, semantic IDs, complete-file SHA, signatures, registry order/relations, field closure, and negative mutations, including the graph-pinned bootstrap root, root-signed currentness proof, key-bound current-verifier authority ID, root/verifier key separation, REAL validity ordering and 604800-second ceiling, current-time window, root-signed six-field subject-registry admission, replayable equivalence audit, and stable-subject derivation of `PhaseFReviewerActorAttestationV1`. | §§44, 49, 53.4–53.5, 53.9–53.10 |
...
| <a id="F-CNF-005"></a>`F-CNF-005` | `F-ARCH-017,F-ARCH-022,F-WIRE-008` | Constructive audits verify the explicit authority DAG, the exact production-derived external reviewer-bootstrap trust dependency set, immutable proof/root history and monotonic accepted-head checkpoint, target-relative historical issuance resolution versus current authorization, no future/self/reference cycles, schema/anchor/catalog equality, exhaustive usage matrix, traceability inverse, closed test/evidence/KAT catalogs, owner-decision union, Markdown structure, no duplicate current traceability graph, REAL reviewer actor attestation resolution/independence, and complete private-manifest anti-alias auditing. | §§53.6–53.13 |
...
| R12-G3-REAL-FORMAT-POSITIVE | constructive_plan_audit | isolated_real_repository_authority | isolated repository with canonical authority JSON, annotated tags, real Git target, and canonical GitHub `refs/heads/main` exactly equal to the selected target; documented raw GitHub `version_id` history and individual-version responses routed through `GitHubApiTransport`; complete protection-digest preimage | raw production-shaped GitHub transport fixture plus real-format fixture | `GitHubApiTransport.get_ref(...); GitHubApiTransport.get_ruleset_history(...); GitHubApiTransport.get_ruleset_version(...); make_repository_context(...); validate_g3_tag(...)` | PASS | canonical publication is read at start and again immediately before resolved REAL GO | 0 |
| R12-G3-REAL-FORMAT-NEGATIVE-MATRIX | constructive_plan_audit | isolated_real_repository_authority | raw history/version mutations plus canonical publication main absent, unavailable, malformed, wrong repository identity, unequal target, stale ancestor, local-only descendant, mid-resolution head change, mutable local remote/SSH changes, publication resolver removal/replacement, and all prior real-format mutations | raw production-shaped GitHub transport fixture and fully populated REAL-format publication/protection/authority mutation matrix | `GitHubApiTransport.get_ref(...); make_repository_context(...); resolve_external_dependency(...); validate_g3_tag(...)` | REJECT | no caller-selected or local Git state can substitute for canonical GitHub publication; every prior negative remains closed | 0 |
...
| R12-G3-REAL-CURRENTNESS-POLICY-NEGATIVE-MATRIX | constructive_plan_audit | isolated_real_repository_authority | production-format REAL proof with key-bound verifier identity, distinct root/verifier keys, exact boundary lifetime, five-role subject population, and external evidence-hash bindings | arbitrary/wrong-key/wrong-fingerprint verifier IDs, stale-ID verifier rotation, root key/fingerprint reuse, verifier-signed proof, zero/reverse/604801-second/multi-year/expired/future windows, empty/duplicate/evidence-alias/malformed subject bindings | `_validate_reviewer_bootstrap_proof_object(..., require_current_window=True)` | REJECT | all unauthorized or malformed policy cases reject; the unmodified REAL fixture with exactly 604800 seconds passes | 0 |
| R12-G3-REAL-HISTORICAL-RESOLUTION | constructive_plan_audit | isolated_real_repository_authority | explicit `HISTORICAL_VALIDATION` proves a target in canonical main lineage and retains exact issuance proof/root provenance after main/currentness advances; historical mode cannot be consumed as current authority | canonical main lineage `A -> B`, historical target `A`, local-only unrelated target negative, proof-0 artifact after proof-2 advancement, and attempted G3 consumption | `make_repository_context(..., resolution_purpose=HISTORICAL_VALIDATION); validate_historical_review_artifact(...); validate_g3_tag(...)` | PASS for historical cryptographic validity; REJECT for current operational GO | historical validity is non-authorizing and exact current-main equality remains mandatory for current mode | 0 |
| R12-G3-REAL-HISTORICAL-CURRENTNESS-POLICY | constructive_plan_audit | isolated_real_repository_authority | checked-in production-path regression proves the selected historical target owns its legacy currentness policy, accepts an expired 604801-second proof only as historical evidence, rejects the equivalent proof under the current target policy, propagates `HISTORICAL_VALIDATION` through nested contexts, and rejects purpose escalation | full isolated REAL-format Git repository with target-relative graph, root/proof history, subject registry, actor attestations, publication lineage, legacy-policy descendant targets, and current-policy lifetime negative | `python3 -B tests/phase_f_historical_bootstrap_currentness.py` | PASS for historical cryptographic validity with `currently_authorized=false`; REJECT for current policy and purpose mutation | policy is selected from the immutable target graph; no target-SHA allowlist and no historical-to-current promotion | 0 |
...
| R12-DAG-EXTERNAL-TRUST-CLOSURE | constructive_plan_audit | external_trust_dependency_graph | the single production resolver dependency registry derives the exact 22-node/27-edge external set, including canonical GitHub repository identity, immutable ruleset protection evidence, published target, protected monotonic head, signed subject-registry state, accepted-head cache, reviewer identity, review artifact, and downstream gates | every required registered node and edge omitted once, registry omission, typed hook/discovery mutation, protection-policy mutation, plus unauthorized future node/edge insertions | `_external_trust_dependency_audit(...); resolve_external_dependency(...); _require_external_dependency(...)` | REJECT | zero accepted omissions, registry drift, unauthorized dependencies, or unverified protection; stage-monotonic acyclic graph remains required | 0 |
...
canonical publication main absence, mismatch, unavailability, repository
identity, local-only and stale targets, start/end races, and publication
resolver registration in addition to missing objects, malformed/noncanonical
objects, wrong digests, wrong targets,
...
equality for 22 nodes and 27 edges. Canonical GitHub repository identity is a
root prerequisite of the strengthened `published_normative_target`, whose
registered production hook reads `refs/heads/main`; that published target in
turn precedes the protected external monotonic head. The closure also includes
authenticated immutable ruleset protection evidence, signed subject-registry
state, immutable bootstrap history, accepted-head cache, reviewer identity,
review artifact, and downstream gates. The production registration is the sole
source for this projection; every required node and edge is omitted once in the
self-test, typed hook/discovery and protection mutations are independently
tested, and each omission rejects. Unauthorized future dependencies reject as
well. R11-08 closure therefore requires both the 28-node/76-edge core graph
and this external trust-DAG closure.
...
carried reviewer digest equals the exact domain-separated subject derivation,
that each attestation binds the complete root/proof references and one
permitted role, and that all five actor digests remain distinct. The fixture is
explicitly `real_test`; it is not a real approval or review artifact.
...
- wrong actor class, role, verifier, root/proof ID/hash/key, or evidence
  binding;
- stale, revoked, compromised, superseded, or invalidated root/proof/
  attestation state;
- one actor represented by different reviewer IDs, different keys, different
  attestations, or different bootstrap subject/proof records;
...
The currentness-policy matrix additionally exercises the production proof
validator with a REAL authority class. The verifier ID is recomputed from the
exact public-key/fingerprint JCS pair under
`mhi_phase_f_reviewer_bootstrap_verifier_authority_v1\0`; arbitrary IDs,
wrong-key IDs, wrong-fingerprint IDs, root-key reuse, root-fingerprint reuse,
and a currentness signature made by the verifier all reject. The unmodified
REAL fixture is a positive boundary case with exactly `604800` seconds. Zero,
reverse, `604801`-second, multi-year, expired, and future windows reject, as do
empty, duplicate, evidence-alias, and malformed subject bindings. Historical
...
file without those proofs is not a positive case. This matrix proves the
identity boundary, not the legitimacy of any actual person; real onboarding
still requires a separately authorized external bootstrap authority and
independently reviewed/published revision.
...
cells and verifies both directions against the 99-schema catalog.
