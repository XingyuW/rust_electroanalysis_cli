...
| FR-041 | Complete prediction uncertainty shall require caller-supplied runtime covariance for all relevant stochastic sources, derivative coverage, observation variance, and finite values | High | `src/model/compiler.rs`, `src/model/output.rs` | uncertainty status regressions | ✅ Implemented |
...
| SCI-013 | Scalar ISM prediction variance shall use caller-supplied runtime `Jx Px Jx^T + Jtheta Ptheta Jtheta^T + R`, retaining covariance cross terms without treating declared uncertainty as posterior covariance | `src/model/compiler.rs` | built-in and model-core covariance tests |
